 ...
 *    打孔提前量 600ms；曲首强制归位：连按 7 次 ◀ 钉底 A0（⌈88/14⌉，从任意位置保证到底）
...
(function (global) {
  ...
  var HOME_PRESSES = Math.ceil(TOP_SLOT / SLOT_STEP); // 7：从任意位置连按 ◀ 钉底 A0（每按 −1 档，止点钳位）
   ...
   * @param opts.home 曲首强制归位：连按 7 次 ◀ 钉底到初始档位 A0（键盘初始位置不确定）
   ...
   * home：归位孔数（0 或 7，shifts 不计入归位）；homeOk：归位末次按动 + 升挡链能否赶在首音前
   *
  ...
  function scheduleRow(notes, row, opts) {
    ...
    // 曲首强制归位：连按 7 次 ◀ 钉底到 A0（W_MAX=7：3 挡不够，⌈88/14⌉=7 从任意位置保证到底；
    // 间隔 ≥400ms 且同轨孔距 ≥minGapMM）。
